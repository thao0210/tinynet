...
const Champions = () => {
  ...
  const getMyResults = async () => {
    try {
      ...
      if (getMyResults.data && getMyResults.data.hasNewAward) {
        setMyAward(getMyResults.data.awards || []);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const returnContentHtml = (awards) => {
    return (
      <ul>
  ...
  const totalPoints = myAward?.reduce((sum, item) => sum + item.points, 0) || 0;
            ...
            {results.map((item, index) => 
              <li 
                ...
                className={classNames(
                  classes['rank' + (index + 1)], 
                  { [classes.myResult]: myRank === (index + 1) && !item.author }
                )}
              >
                <span className={classes.rank}>{index > 2 ? index + 1 : ''}</span>
                {item.title && (
                  <div className={classes.box} onClick={() => openPost(item._id)}>
                    {item.title}
                  </div>
                )}
                {item.content && (
                  <div className={classes.box} onClick={() => openPost(item.itemId)}>
                    {item.content}
                  </div>
                )}
                <div className={classes.author}>
                  {item.author ? (
                    <>
                      ...
                      {item.author.fullName ? item.author.fullName : item.author.username}
                    </>
                  ) : (
                    <>
